...
public protocol LightBoxActionDelegate: class {
    func lightboxControllerDidRequestDelete(_ controller: LightboxController, at index: Int)
    func lightboxControllerDidRequestSelect(_ controller: LightboxController, at index: Int)
}
...
open class LightboxController: UIViewController {
    ...
    open weak var lightBoxActionDelegate: LightBoxActionDelegate?
...
extension LightboxController: HeaderViewDelegate {
    ...
    func headerView(_ headerView: HeaderView, didPressLeftButton leftButton: UIButton) {
        ...
        switch LightboxConfig.LeftButton.buttonType {
        case .select:
            handleSelect(leftButton)
            
        case .delete:
            handleDelete(leftButton)
        }
    }
...
extension LightboxController: FooterViewDelegate {
    ...
    private func handleSelect(_ leftButton: UIButton) {
        leftButton.isEnabled = false

        lightBoxActionDelegate?.lightboxControllerDidRequestSelect(self, at: currentPage)
        handleDismiss()
    }
    
    private func handleDelete(_ leftButton: UIButton) {
        leftButton.isEnabled = false
        lightBoxActionDelegate?.lightboxControllerDidRequestDelete(self, at: currentPage)
